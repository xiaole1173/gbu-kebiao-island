...
### 🔐 教务同步（仅同步本人课表）
- 大湾区大学统一身份认证 SSO 登录（iaaa.gbu.edu.cn），免验证码一键通过
...
- 闹钟式全屏提醒：响铃 + 全屏 + 手动关闭（后台 / 锁屏用 fullScreenIntent 保证弹出）
- "当前 / 下一节课"常驻通知（可为灵动岛 / 胶囊提供数据），每次触发后自动重排
